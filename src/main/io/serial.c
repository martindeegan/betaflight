...
const uint32_t
    baudRates[] = {0,       9600,    19200,   38400,  57600,  115200,
                   230400,  250000,  400000,  460800, 500000, 921600,
...
void pgResetFn_serialConfig(serialConfig_t *serialConfig) {
...
#ifdef SERIALRX_UART
  serialPortConfig_t *serialRxUartConfig =
  ...
  serialPortConfig_t *serialTlemetryUartConfig =
...
#define USE_MSP_UART
  ...
  if (serialConfig->portConfigs[0].identifier == SERIAL_PORT_USB_VCP) {
    serialPortConfig_t *uart1Config =
        serialFindPortConfiguration(SERIAL_PORT_UART4);
    if (uart1Config) {
...
serialPortConfig_t *findNextSerialPortConfig(serialPortFunction_e function) {
  while (findSerialPortConfigState.lastIndex < SERIAL_PORT_COUNT) {
    serialPortConfig_t *candidate =
        &serialConfigMutable()
             ->portConfigs[findSerialPortConfigState.lastIndex++];
...
portSharing_e determinePortSharing(const serialPortConfig_t *portConfig,
...
bool isSerialPortShared(const serialPortConfig_t *portConfig,
  ...
  return (portConfig) && (portConfig->functionMask & sharedWithFunction) &&
         (portConfig->functionMask & functionMask);
}

serialPort_t *findSharedSerialPort(uint16_t functionMask,
                                   serialPortFunction_e sharedWithFunction) {
  for (unsigned i = 0; i < SERIAL_PORT_COUNT; i++) {
    ...
    if (isSerialPortShared(candidate, functionMask, sharedWithFunction)) {
      const serialPortUsage_t *serialPortUsage =
...
#ifdef USE_TELEMETRY
#define ALL_FUNCTIONS_SHARABLE_WITH_MSP                                        \
  (FUNCTION_BLACKBOX | TELEMETRY_PORT_FUNCTIONS_MASK)
#else
...
bool isSerialConfigValid(const serialConfig_t *serialConfigToCheck) {
   ...
   *   (using either / or, switching based on armed / disarmed or the AUX
   * channel configured for BOXTELEMETRY)
  ...
  for (int index = 0; index < SERIAL_PORT_COUNT; index++) {
    const serialPortConfig_t *portConfig =
        &serialConfigToCheck->portConfigs[index];
    ...
    if (bitCount > 1) {
      ...
      if ((portConfig->functionMask & FUNCTION_MSP) &&
          (portConfig->functionMask & ALL_FUNCTIONS_SHARABLE_WITH_MSP)) {
        // MSP & telemetry
#ifdef USE_TELEMETRY
...
serialPort_t *openSerialPort(serialPortIdentifier_e identifier,
  ...
  case SERIAL_PORT_USART8:
    ...
    serialPort =
        serTcpOpen(SERIAL_PORT_IDENTIFIER_TO_UARTDEV(identifier), rxCallback,
...
void serialInit(bool softserialEnabled,
  ...
  for (int index = 0; index < SERIAL_PORT_COUNT; index++) {
    ...
    if (serialPortToDisable != SERIAL_PORT_NONE) {
      ...
      int resourceIndex = SERIAL_PORT_IDENTIFIER_TO_INDEX(
          serialPortUsageList[index].identifier);
      if (!(serialPinConfig()->ioTagTx[resourceIndex] ||
            serialPinConfig()->ioTagRx[resourceIndex])) {
        serialPortUsageList[index].identifier = SERIAL_PORT_NONE;
        serialPortCount--;
      }
    }
...
#ifdef USE_SOFTSERIAL1
         && !(softserialEnabled &&
              (serialPinConfig()->ioTagTx[RESOURCE_SOFT_OFFSET + SOFTSERIAL1] ||
...
#ifdef USE_SOFTSERIAL2
         && !(softserialEnabled &&
              (serialPinConfig()->ioTagTx[RESOURCE_SOFT_OFFSET + SOFTSERIAL2] ||
...
static void nopConsumer(uint8_t data) { UNUSED(data); }
...
void serialPassthrough(serialPort_t *left, serialPort_t *right,
                       serialConsumer *leftC, serialConsumer *rightC) {
